...
public class UrlPingerMonitorTest {
    ...
    public static final String SAMPLE_HTTP_HOST = "http://221.10.40.234";
    public static final String SAMPLE_HTTP_PORT = "843";
    public static final String ADMIN = "admin";

    JAXBProvider<MonitorUrls> jaxbProvider = mock(JAXBProvider.class);
    MonitorUrls monitorUrls;

    @Before
    public void setup() throws JAXBException {
        createData();
        when(jaxbProvider.unmarshal(anyString(), (Class)any())).thenReturn(monitorUrls);
    }
    ...
    public void testUrlPingerExtension() throws TaskExecutionException {
        UrlPingerMonitor pingerMonitor = new UrlPingerMonitor(Executors.newFixedThreadPool(NUMBER_OF_THREADS),jaxbProvider);
        Map<String,String> taskArgs = Maps.newHashMap();
        TaskOutput output = pingerMonitor.execute(taskArgs, null);
        assert(output.getStatusMessage().contains("successfully"));
    }
    ...
    public void testUrlPingerExtensionWithHttpProxy() throws TaskExecutionException {
        UrlPingerMonitor pingerMonitor = new UrlPingerMonitor(Executors.newFixedThreadPool(NUMBER_OF_THREADS),jaxbProvider);
        Map<String,String> taskArgs = Maps.newHashMap();
        taskArgs.put(MonitorArgs.PROXY_HOST, SAMPLE_HTTP_HOST);
        taskArgs.put(MonitorArgs.PROXY_PORT, SAMPLE_HTTP_PORT);
        TaskOutput output = pingerMonitor.execute(taskArgs, null);
        assert(output.getStatusMessage().contains("successfully"));
    }
